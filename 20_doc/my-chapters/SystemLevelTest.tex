...
\begin{itemize}[label=-, leftmargin=1.5cm]
	\item Đầu tiên là về số lượng sử dụng phép so sánh và phép swap:
	...
	\begin{table}[H]
		\centering
		{
			...
			\begin{tabular}{@{}p{4cm} p{5.5cm} p{5.5cm}@{}}
				...
				& \textbf{Iterative (C\_Framework\_RTL)} 
				& \textbf{Recursive (C\_Framework\_Standard)} \\
				\midrule
				...
				\textbf{Number of Compare} 
				& 5918523 
				& 1182680610 \\
				
				\textbf{Number of Swap} 
				& 2957092 
				& 581537292 \\
		...
		\caption{So sánh đặc tính sử dụng bộ so sánh và swap giữa mô hình Framework sử dụng Iterative (RTL) và Recursive (Standard).}
		\label{tab:number_compare_swap_iterative_recursive}
	\end{table}
	
	\item Thứ hai là về sử dụng bộ nhớ của đoạn code: 
	...
	\begin{table}[H]
		\centering
		{
			...
			\begin{tabular}{@{}p{4cm} p{5.5cm} p{5.5cm}@{}}
				...
				\textbf{Peak Memory (Đỉnh)} 
				& 8.090 MB 
				& 8.662 MB \\
				
				\textbf{Chênh lệch bộ nhớ} 
				& Thấp hơn 
				& Cao hơn khoảng 0.57 MB \\
				
				\textbf{Thời điểm đạt đỉnh} 
				& Snapshot 41 \newline (giữa quá trình) 
				& Snapshot 86 \newline (gần cuối quá trình) \\
				
				\textbf{Hình dạng biểu đồ} 
				& Cao nguyên (Plateau): \newline tăng lên và giữ ổn định 
				& Gai nhọn (Spike): \newline duy trì thấp rồi tăng đột ngột \\
				
				\bottomrule
			\end{tabular}
		}
		\caption{So sánh đặc tính sử dụng bộ nhớ giữa mô hình Framework sử dụng Iterative (RTL) và Recursive (Standard).}
		\label{tab:memory_iterative_recursive}
	\end{table}
\end{itemize}