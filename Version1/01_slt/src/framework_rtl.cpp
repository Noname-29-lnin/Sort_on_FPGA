...
IndexType C_Framework_RTL::P_Partition(std::vector<SIZE_TYPE_T> &arr, IndexType si, IndexType ei){
...
void C_Framework_RTL::P_Division(std::vector<SIZE_ARR_T>& arr, IndexType si, IndexType ei, int M, int S_cnt){
    if(si < ei){
        if(S_cnt < (1 << (M-1))){
